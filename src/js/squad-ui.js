...
export async function initSquadUI() {
...
function viewSquadDetails(squadId) {
    ...
    const players = isUnassigned
        ...
        : sortPlayersByPosition(squadManager.getPlayers({ squadId: squadId }));
    ...
    // Inject view toggle into the top row (next to back button)
                ...
                grouped.forEach(group => {
                    html += `<div class="squad-position-group">
                        <div class="squad-group-header">
                            <span class="squad-group-label">${group.label}</span>
        ...
        if (tableWrapper) {
            ...
            if (tbody) {
                if (players.length === 0) {
                    ...
                    let html = '';
                    grouped.forEach(group => {
                        html += `<tr class="squad-group-header-row">
                            <td colspan="7">
                                <span class="squad-group-label">${group.label}</span>
                                <span class="squad-group-count">${group.players.length}</span>
                            </td>
                        </tr>`;
                        html += group.players.map(p => renderPlayerRow(p, true)).join('');
                    });
...
function groupPlayersByPosition(players) {
    ...
    return groups
        .map((players, i) => ({ label: labels[i], players }))
...
function renderSquads() {
    ...
    // Build unassigned players card
    const unassignedPlayers = squadManager.getPlayers({}).filter(p => !p.squadId);
    ...
    const totalDisplay = squads.length + (showUnassigned ? 1 : 0);
    if (count) count.textContent = `${totalDisplay} squads`;
    ...
    if (squads.length === 0 && !showUnassigned) {
        ...
        // Append unassigned card at the end
        if (showUnassigned) {
            html += `
            <div class="dash-card squad-card" onclick="viewSquadDetails('unassigned')" style="cursor: pointer; transition: transform 0.2s; padding: 24px; position: relative; border: 1px dashed #cbd5e1;">
                <div style="display: flex; justify-content: space-between; align-items: flex-start; margin-bottom: 16px;">
                    <div class="icon-circle" style="width: 48px; height: 48px; background: #fef3c7; color: #d97706; display: flex; align-items: center; justify-content: center; border-radius: 12px; font-size: 1.25rem;">
                         <i class="fas fa-user-clock"></i>
                    </div>
                </div>
                <h3 style="margin: 0 0 8px 0; font-size: 1.2rem; color: var(--navy-dark);">Unassigned</h3>
                <div style="color: var(--text-secondary); font-size: 0.9rem; margin-bottom: 20px;">
