...
/** Mean of all rated attributes across the nested ratings object, to 1 decimal. */
export function computeGlobalAverage(ratings: Record<string, Record<string, number>>): number | null {
  const vals: number[] = [];
  for (const sec of Object.values(ratings || {})) {
    for (const v of Object.values(sec || {})) { if (typeof v === 'number' && v > 0) vals.push(v); }
  }
